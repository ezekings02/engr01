...
public class TextIO {
	...
	/*
	 * Modified November 2007 to empty the TextIO input buffer when switching
	 * from one input source to another. This fixes a bug that allows input from
	 * the previous input source to be read after the new source has been
	 * selected.
	 */
	 ...
	 * character and can be the end-of-file character (specified by the constant
	 ...
	 * error. (Ordinarily, end-of-file is not possible when reading from
	 * standard input.)
	 */
	 ...
	 * (Ordinarily, end-of-file is not possible when reading from standard
	 * input.)
	 */
	private static void skipWhitespace() {
		char ch = lookChar();
		while (ch != EOF && Character.isWhitespace(ch)) {
			...
			if (ch == '\n' && readingStandardInput && writingStandardOutput) {
				// out.print("? "); //edit by TeM 140113 to remove the dreaded
				// questionmark problem
				out.flush();
			}
			ch = lookChar();
		}
	...
	/**
	 * Reads all the characters from the current input source, up to the next
	...
	private final static BufferedReader standardInput = new BufferedReader(
			new InputStreamReader(System.in)); // wraps standard input stream
	private final static PrintWriter standardOutput = new PrintWriter(
			System.out); // wraps standard output stream
	...
	private static int outputErrorCount; // Number of errors on standard output
	// since it was selected as the
	// output destination.
	...
	private static String readRealString() { // read chars from input following
	...
	private static void errorMessage(String message, String expecting) { // Report
		...
		if (readingStandardInput && writingStandardOutput) {
			...
			if (lookChar() == '\n')
				...
				while (lookChar() != '\n')
					// Discard and echo remaining chars on the current line of
					// input.
					out.print(readChar());
				out.print("\n\n");
			}
	...
	private static void fillBuffer() { // Wait for user to type a line and press
		// return,
		try {
			buffer = in.readLine();
		} catch (Exception e) {
	...
	private static void emptyBuffer() { // discard the rest of the current line
		// of input
		buffer = null;
	}
